...
public class PhotonOdometryCameraSim implements OdometryCamera {
    ...
    public PhotonOdometryCameraSim(PhotonCamera camera, Transform3d robotToCamera, boolean enabled, Function<VisionOdometryPoseEstimate, Matrix<N3, N1>> stdDevProvider) {
        this.m_camera = camera;
        this.m_enabled = enabled;

        this.m_cameraSim = new PhotonCameraSim(camera);
        this.m_cameraSim.enableDrawWireframe(true);
        this.m_cameraSim.enableProcessedStream(true);
    ...
    public synchronized Optional<VisionOdometryPoseEstimate> getEstimate() {
        ...
        Optional<EstimatedRobotPose> estimatedPose = m_poseEstimator.update(this.m_cameraSim.getCamera().getAllUnreadResults().get(0));
        ...
        for (int i = 0; i < estimatedPose.get().targetsUsed.size(); i++) {
            ...
            avgTargetDist += Math.sqrt(
                transform.getTranslation().getX() * transform.getTranslation().getX() +
                transform.getTranslation().getY() * transform.getTranslation().getY() +
                transform.getTranslation().getZ() * transform.getTranslation().getZ()
            );
        }
